...
@section('content')
    ...
    <form action="{{ route('dehum-matras.update', $check->hashid) }}" method="POST" autocomplete="off">
        ...
        @php
            ...
            // Helper function untuk mendapatkan checker_id berdasarkan tanggal
            ...
            function getApprovedBy($results, $date) {
                $result = $results->where('tanggal', $date)->where('approved_by', '!=', null)->first();
                        ...
                        <tr class="bg-sky-50">
                            ...
                            @for($j = 1; $j <= 11; $j++)
                                ...
                                <td class="border border-gray-300 p-1 {{ $rowClass }}">
                                    <div x-data="{ 
                                        selected: {{ wasCheckedByUser($results, $j) ? 'true' : 'false' }}, 
                                        userName: '{{ getCheckerName($results, $j) }}', 
                                        isExistingData: {{ !empty(getCheckerName($results, $j)) ? 'true' : 'false' }}, 
                                        ...
                                        <div x-show="isExistingData" class="w-full px-2 py-1 text-sm {{ $isReadOnly ? 'bg-green-100' : 'bg-white' }} border border-gray-300 rounded text-center">
                                            ...
                                            <input type="hidden" name="checker_id_{{ $j }}" value="{{ getCheckerId($results, $j) }}">
                                        </div>
                                        
                                        <!-- Show the form if selected but not existing data -->
                                        <div x-show="selected && !isExistingData && !isReadOnly" class="w-full">
                                            <input type="text" name="checker_username_{{ $j }}" x-ref="user{{ $j }}" x-bind:value="userName"
                                                class="w-full px-2 py-1 text-sm bg-white border border-gray-300 rounded text-center"
                                                readonly>
                                            <input type="hidden" name="checker_id_{{ $j }}" x-ref="checker_id{{ $j }}" value="">
                                        </div>
                                        ...
                                        @if($isReadOnly)
                                            ...
                                            <input type="hidden" name="approver_id_{{ $j }}" value="{{ getApprovedBy($results, $j) }}">
                                        @endif
                                            ...
                                            <button type="button" 
                                                @click="
                                                    ...
                                                    if(selected) {
                                                        ...
                                                        $refs['user' + cellId].value = userName;
                                                        $refs['checker_id' + cellId].value = '{{ $user->id }}';
                                                    } else {
                                                        userName = '';
                                                        if($refs['user' + cellId]) $refs['user' + cellId].value = '';
                                                        if($refs['checker_id' + cellId]) $refs['checker_id' + cellId].value = '';
                                                    }
                                                "
